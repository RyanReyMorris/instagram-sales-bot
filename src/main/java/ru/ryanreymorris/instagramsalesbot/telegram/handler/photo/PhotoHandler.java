package ru.ryanreymorris.instagramsalesbot.telegram.handler.photo;
...
public class PhotoHandler implements UpdateHandler {
    ...
    public void handle(Update update) {
        ...
        switch (botUser.getActiveButton()) {
            case MASS_SENDING -> {
                SendPhoto sendPhoto = new SendPhoto("", new InputFile(update.getMessage().getPhoto().get(0).getFileId()));
                sendPhoto.setCaption(update.getMessage().getCaption());
                massSendingStorage.setMethod(sendPhoto);
