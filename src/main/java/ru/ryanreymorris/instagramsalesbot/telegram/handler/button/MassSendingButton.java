...
@Component
public class MassSendingButton implements Button {

    private final static String MASS_SENDING = "Введите сообщение, которое будет направлено ВСЕМ пользователям";
    ...
    public void handleClick(Update update) {
        ...
        SendMessage sendMessage = replyMessagesService.createMessage(MASS_SENDING, botUser.getTgUserId());
        botMessageService.updateLastMessage(sendMessage, update);
    }

    @Override
    public ButtonEnum getButton() {
        return ButtonEnum.MASS_SENDING;
    }
}
